...
| `--time-budget` | `240` | Wall-clock seconds the script may use. Set it to ~70% of the per-command timeout. Models are trained in priority order and any that do not fit are skipped, so a short budget still yields a valid submission. |
...
- Before training begins the script writes a constant-prior submission, so a valid file exists
  even if the run is later killed by a timeout.
