...
def build_models(n_rows: int, n_feats: int, seed: int):
    ...
    try:
        ...
        models.append(("lgbm_deep", lambda a, b, c, d, e: fit_lgb(a, b, c, d, e, deep), 1.2))
...
def run_cv(name, fn, X, y, T, n_splits, seed):
...
def main() -> int:
    ...
    for name, fn, cost in models:
        ...
        try:
            t0 = time.time()
            auc, oof, tp = run_cv(name, fn, X, y, T, n_splits, args.seed)
