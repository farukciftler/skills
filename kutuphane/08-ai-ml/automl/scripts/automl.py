...
def read_csv_robust(path: str) -> pd.DataFrame:
    """Read a CSV without letting a parser bug kill the process.

    The default C parser segfaults on at least one dataset of this family
    (train_14); the pyarrow engine does not. Try pyarrow first and fall back
    if it is unavailable or fails.
    """
    try:
        return pd.read_csv(path, engine="pyarrow")
    except Exception as exc:
        log(f"pyarrow read failed for {os.path.basename(path)} ({type(exc).__name__}); "
...
def main() -> int:
    ...
    train = read_csv_robust(os.path.join(data_dir, "train.csv"))
    test = read_csv_robust(os.path.join(data_dir, "test.csv"))
    sub = read_csv_robust(os.path.join(data_dir, "sample_submission.csv"))
    ...
    def write_preds(preds: np.ndarray, tag: str) -> None:
        preds = np.asarray(preds, dtype=float)
        if not np.isfinite(preds).all():
            preds = np.nan_to_num(preds, nan=float(y.mean()))
        out = sub.copy()
        out[target_col] = preds
        out.to_csv(out_path, index=False)
        log(f"checkpoint[{tag}] -> {out_path}")

    # Fast first pass: replace the constant prior with real predictions within
    # seconds, so a kill at any later point still leaves a scoring submission.
    try:
        from sklearn.model_selection import train_test_split
        from sklearn.metrics import roc_auc_score as _ras
        Xq, Tq = X.to_numpy(), T.to_numpy()
        tr_i, va_i = train_test_split(np.arange(len(Xq)), test_size=0.15,
                                      stratify=y, random_state=args.seed)
        try:
            import lightgbm as _lgb
            qm = _lgb.LGBMClassifier(n_estimators=300, learning_rate=0.08, num_leaves=31,
                                     subsample=0.9, colsample_bytree=0.8, random_state=args.seed,
                                     n_jobs=-1, verbose=-1)
        except Exception:
            from sklearn.ensemble import HistGradientBoostingClassifier as _HGB
            qm = _HGB(max_iter=300, learning_rate=0.1, random_state=args.seed)
        qm.fit(Xq[tr_i], y[tr_i])
        q_auc = _ras(y[va_i], qm.predict_proba(Xq[va_i])[:, 1])
        write_preds(qm.predict_proba(Tq)[:, 1], f"fast_pass holdout_auc={q_auc:.5f}")
    except Exception as exc:
        log(f"fast pass FAILED (prior submission stays): {type(exc).__name__}: {exc}")
    ...
    def blend_current():
        """Blend everything trained so far; return (final_preds, blend_auc, weights)."""
        names_now = list(oofs)
        if len(names_now) == 1:
            k = names_now[0]
            return tests[k], scores[k], {k: 1.0}
        w, b_auc = hill_climb(oofs, y, names_now)
        fin = np.zeros(len(T))
        for k, v in w.items():
            fin += v * rank01(tests[k])
        best_k = max(scores, key=scores.get)
        if b_auc < scores[best_k] - 1e-6:
            return tests[best_k], scores[best_k], {best_k: 1.0}
        return fin, b_auc, w
    ...
    for name, fn, cost in models:
        ...
        try:
            ...
            # Progressive checkpoint: a kill during any later model leaves the
            # best blend of everything finished so far, never a stale file.
            try:
                ck_final, ck_auc, _ = blend_current()
                write_preds(ck_final, f"after_{name} blend_oof_auc={ck_auc:.5f}")
            except Exception as exc:
                log(f"checkpoint FAILED: {type(exc).__name__}: {exc}")
    ...
    if not oofs:
        log("No CV model succeeded; the fast-pass (or prior) submission stands.")
        return 1

    final, blend_auc, weights = blend_current()
