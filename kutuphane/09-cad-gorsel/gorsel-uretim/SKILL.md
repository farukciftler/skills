...
```bash
python3 scripts/aksam.py cephe.jpg cephe-aksam.jpg --gokyuzu mavi-saat
```

Gündüz render'ını **mavi saat / gün batımı** görünümüne çevirir: gökyüzünü
değiştirir, cepheyi karartır, pencereleri yakar. `gorsel-yon-referans.md`'deki
K1 kuralının ara çözümü (kaynak görsel katalogdan çıkarılmış olamaz, bkz. §5) — gerçek çözüm mimari ekipten akşam render pass'i
istemektir. `--gokyuzu gun-batimi` daha sıcak, `--yogunluk` gökyüzü gücü,
`--karartma` cephe karartması.
