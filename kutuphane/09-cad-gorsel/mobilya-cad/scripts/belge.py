...
# WinAnsi'dir ve ş/ğ/ı taşımaz. Aday (normal, kalın, italik) üçlüleri
# sırayla denenir; MOBILYA_FONT_DIR verilirse önce oraya bakılır.
_FONT_ADAYLARI = [
...
_FONT_KLASORLERI = [
    os.environ.get("MOBILYA_FONT_DIR", ""),
...
def _font_bul():
    for kl in _FONT_KLASORLERI:
        if not kl or not os.path.isdir(kl):
            continue
        for uclu in _FONT_ADAYLARI:
            yollar = [os.path.join(kl, ad) for ad in uclu]
            if all(os.path.exists(y) for y in yollar):
                return yollar
    return None


def _fontlari_yukle():
    if "Gov" in pdfmetrics.getRegisteredFontNames():
        return
    yollar = _font_bul()
    if not yollar:
        raise FileNotFoundError(
            "Türkçe destekli TTF bulunamadı. Arial veya DejaVu Sans kurun ya "
            "da MOBILYA_FONT_DIR ile Arial.ttf / Arial Bold.ttf / "
            "Arial Italic.ttf içeren klasörü gösterin.")
    for ad, yol in zip(("Gov", "GovB", "GovI"), yollar):
