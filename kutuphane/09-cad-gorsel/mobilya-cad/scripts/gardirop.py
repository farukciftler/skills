...
kendi `Bolge` yığınını taşır. Yan panel, tabla, kapak, baza ve arkalık
parça/yerleşim yardımcıları, rapor iskeleti, çizim, render ve belge
dolap.py ve mobilya.py'den AYNEN gelir — burada yalnız sütun düzeni,
kontroller ve sütuna özgü parçalar (dikme, kuşak, çekmece, sırt paneli,
ayna) vardır.
...
Bölge tipleri: dolap.Bolge'dekilere ek olarak
  "cekmece" — raf sayısı kadar çekmece (ön panel BOM'a, ray donanıma girer)

Ek kavramlar:
  ortak_alt  — tüm sütunların altında dikmesiz, tam genişlik bölme
               (yorgan/valiz). Tavanı ön KUŞAKLA taşınır (kusak=100).
  Sutun(ust_yukseklik=0) — bu sütunun üstünde dolap yok: gövde L olur,
               açık alanın arkasında 18 mm sırt paneli + duvar braketi.
"""
...
from mobilya import (
    ASKI, aski_kapasitesi, aski_yonu, MALZEMELER, SISTEM32, Parca,
    devrilme_kontrolu, kusakli_sehim, max_aciklik, raf_sehimi,
)
from dolap import (
    Bolge, Dolap, DERZ, KIYAFET_BOYU_KONTROL, ARKALIK_TASMA, BANT_KAPAK,
)
...
class Sutun:
    ...
    ad: str = ""
    kapak: str | None = None          # None | "tek" | "cift" | "yok"; None
                                      # ise Gardirop.kapak kuralı uygulanır
...
class Gardirop(Dolap):
    def __init__(self, *, genislik, derinlik, yukseklik, sutunlar,
        ...
        # Sütuna özgü alanlar Dolap.__init__'ten ÖNCE — o _hesapla() çağırır.
        ...
        # kapak: "sutun" -> her sütuna TEK kanat · "cift" -> her sütuna ÇİFT
        # (dar) kanat · "tek" -> her sütuna tek kanat · None/"yok" -> kapaksız.
        # Sutun.kapak bu kuralı sütun bazında ezer.
        self.kapak_kurali = kapak
        self.kiyafet = kiyafet
        # tam_kapak=kapak: belge.py dış ölçüde kapağı sayabilsin.
        super().__init__(genislik=genislik, derinlik=derinlik,
                         yukseklik=yukseklik, bolgeler=[], tam_kapak=kapak,
                         baza=baza, baza_icerlek=baza_icerlek,
                         kalinlik=kalinlik, malzeme=malzeme, arkalik=arkalik,
                         ayakkabi_tipi="karma", kod=kod, ad=ad,
                         aciklama=aciklama)

    # ---- sütun geometrisi ------------------------------------------------
    def _sutun_kapak_tipi(self, s: Sutun) -> str | None:
        """Sütunun kanat tipi: Sutun.kapak ezer; yoksa Gardirop.kapak kuralı
        ("sutun" -> tek kanat, "tek"/"cift" -> her sütuna o kadar kanat,
        None/"yok" -> kapaksız)."""
        tip = s.kapak or self.kapak_kurali
        if tip in (None, "yok"):
            return None
        return "tek" if tip == "sutun" else tip
    ...
    def _duzen_ve_kontroller(self):
        ...
        for si, s in enumerate(self.sutunlar):
            if self.sutun_ust[si] <= 0.001:
                ...
                continue
            ic_yuk = self.sutun_ust[si]
            n = len(s.bolgeler)
            kullanilabilir = ic_yuk - (n - 1) * self.T if n else 0.0
    ...
    def _parcalar(self):
        ...
        if not self.L_govde:
            ...
            self.p_yan_sol = self._parca_yan(
                kod, "Yan panel — SOL (tam boy)", boy=hs, adet=1,
                not_="ön kenar 2 mm bant · desen DİKEY")
            self.p_yan = self.p_yan_sol   # CNC delik tablosu bunu kullanır
            self.p_yan_sag = self._parca_yan(
                kod, "Yan panel — SAĞ (alçak)", boy=hd, adet=1,
                bant={"W1": 2.0, "L2": 2.0},
            ...
            ust_gen = [self.sutun_gen[i] for i, u in enumerate(self.sutun_ust)
                       if u > 0.001]
            self.p_ust = self._parca_tabla(
                kod, "Üst tabla (yalnız yüksek sütun)", boy=ust_gen[0],
                adet=len(ust_gen),
        ...
        if self.ortak_alt:
            self.p_ortak_tavan = self._parca_tabla(
                kod, "Ortak bölme tavanı", en=D - 20,
        ...
        if self.dikme_sayisi:
            self.p_dikme = kod()
            if self.L_govde:
                # L gövdede üst tabla yalnız kendi sütununu örter, dikmenin
                # üstüne binmez. Dikme YAN PANEL gibi gövde tepesine kadar
                # çıkar; yoksa üstünde T×T'lik açık çentik kalır ve açık
                # alanın sırt paneli/aynası dikmeden yüksek durur.
                dikme_boy = self.baza + gy - self.sutun_taban
                dikme_bant = {"W1": 2.0, "W2": 2.0}
                dikme_not = (f"YAPISAL — {self.ic_genislik:.0f} mm açıklık "
                             f"dikmesiz {self.max_ack:.0f} mm sınırını aşar. "
                             f"Ortak tavanın üstünden gövde TEPESİNE kadar; "
                             f"üst tabla yanına dayanır (minifix + kavela). "
                             f"Üst kenarı görünür, bantlı.")
            else:
            ...
            P.append(Parca(self.p_dikme, "Orta dikme",
                           ...
                           desen_kilit=True, grup="gövde", bant=dikme_bant,
                           not_=dikme_not))

        # Sütun içi yatay ayırıcılar, raflar, çekmeceler
        self.p_ara = {}
        self.p_raf = {}
        self.p_cekmece = {}
        for si, s in enumerate(self.sutunlar):
            ...
            if ara > 0:
                self.p_ara[si] = self._parca_tabla(
                    kod, f"Sabit ara tabla (S{si+1})", boy=sg, en=D - 20,
                    adet=ara)
        ...
        # Kapaklar — sütun başına
        self.p_kapak = {}
        for si, s in enumerate(self.sutunlar):
            tip = self._sutun_kapak_tipi(s)
            if tip is None:
                continue
            adet = 2 if tip == "cift" else 1
            sg = self.sutun_gen[si]
            # kanat, sütunun kendi genişliği + payına oturur; komşu kanatla
            # arasında TEK derz olduğu için _kanat_genisligi kullanılmaz
            toplam = sg + self.T
            k_gen = (toplam - DERZ - (DERZ if adet == 2 else 0)) / adet
            kapak_boy = gy - (self.sutun_taban - self.baza) - 2 * DERZ
            k = self._parca_kapak(
                kod, f"Kapak (S{si+1})" + (" — AYNALI" if s.ayna else ""),
                kapak_boy, adet,
                "4 kenar 2 mm bant · Ø35 gizli menteşe"
                + (" · ön yüz AYNA, güvenlik filmli 4 mm" if s.ayna else ""),
                gen=k_gen)
            self.p_kapak[si] = (k, adet)
            self.donanim.append(
                (f"{self.kod}-M{si+1}", "Gizli menteşe Ø35 (yavaş kapanan)",
                 "-", adet * self._mentese_adedi(gy),
        ...
        if self.ortak_alt and self.ortak_alt.kapak:
            ...
            k = self._parca_kapak(
                kod, f"Ortak bölme kapağı ({adet} kanat)",
                self.ortak_alt.yukseklik + T - DERZ, adet,
                "Tam genişlik — yorgan gözünü tümüyle kapatır. "
        ...
        if self.L_govde:
            ...
            self.p_ark = self._parca_arkalik(
                kod, "Tam genişlik, ortak bölmenin arkası. "
                     "4 mm kanal içine, 8 mm derin.",
                ad="Arkalık — alt bant", boy=bant_yuk - T + ARKALIK_TASMA)
            yuksek = [i for i, u in enumerate(self.sutun_ust) if u > 0.001]
            if yuksek:
                self.p_ark2 = self._parca_arkalik(
                    kod, "Yalnız dolap olan sütunun arkası. Açık "
                         "alanın arkasında 3 mm HDF YOK — orada "
                         "18 mm sırt paneli var.",
                    ad="Arkalık — yüksek sütun",
                    boy=max(self.sutun_ust) + ARKALIK_TASMA,
                    en=self.sutun_gen[yuksek[0]] + ARKALIK_TASMA,
                    adet=len(yuksek))
        else:
            self.p_ark = self._parca_arkalik(
                kod, "4 mm kanal içine, 8 mm derin, arka kenardan "
    ...
    def _yerlestir(self):
        ...
        for si, (k, bosluk) in self.p_sirt.items():
        ...
        # sütun kapakları (kanat sütun genişliği + T'ye oturur)
        for si, s in enumerate(self.sutunlar):
        ...
        if self.p_ortak_kapak:
            k, adet = self.p_ortak_kapak
            self._yerlestir_kanatlar(k, adet, b0 + DERZ / 2)

        self._yerlestir_baza()

        self._yerlestir_arkalik()
        if self.L_govde and self.p_ark2:
            for si, u in enumerate(self.sutun_ust):
                if u <= 0.001:
                    continue
                self._yerlestir_arkalik(
                    self.p_ark2, x=self.sutun_x[si] - 8,
                    z=b0 + self.sutun_toplam[si] - u - T - 8)

    # ---- belge/çizim arayüzü (Dolap ile uyum) ---------------------------
    @property
    def bolgeler(self):
        """belge.py, uret() ve raf_delikleri() için düzleştirilmiş bölge
        listesi — `kotlar` ile aynı sırada."""
        out = []
        for si, s in enumerate(self.sutunlar):
            for b in s.bolgeler:
    ...
    @bolgeler.setter
    def bolgeler(self, v):
        pass    # kaynak `sutunlar`dır; Dolap.__init__'in ataması yok sayılır
    ...
    # ---- rapor (yalnız sütuna özgü bölümler ezilir) ----------------------
    def _dis_olcu_satiri(self) -> str:
        dis_d = self.D + (self.T if self.p_kapak else 0)
        return (f"Dış ölçü      : {self.G:.0f} G × {dis_d:.0f} D × "
    ...
    def _rapor_duzen(self) -> list[str]:
        ...
        for si, s in enumerate(self.sutunlar):
            kap = {"tek": "tek kanat", "cift": "çift kanat"}.get(
                self._sutun_kapak_tipi(s), "KAPAKSIZ")
