...
# öne bakan askı için 600 mm gövde / ~550 mm net iç derinlik şart.
# ---------------------------------------------------------------------------

ASKI = {
    "askilik_genislik": 450.0,     # standart elbise askısı (430-450)
    "on_bakan_min_ic": 550.0,      # RAHAT öne bakan askı (net iç derinlik)
    "on_bakan_sikisik": 470.0,     # askı sığar ama kalın giysi kapağa değer
...
def aski_yonu(ic_derinlik: float, ic_genislik: float) -> dict:
    ...
    yayılır, ~550 mm net iç derinlik ister.
    Yandan bakan askı (boru önden arkaya): askının genişliği GENİŞLİĞE
    yayılır; derinlik yalnız kaç kıyafet sığacağını belirler.
    """
    ...
    return {
        ...
        "aciklama": (
            ...
            f"{ic_derinlik:.0f} mm, askı {ASKI['askilik_genislik']:.0f} mm — "
            f"{ic_derinlik - ASKI['askilik_genislik']:.0f} mm pay kalıyor. "
            f"Gömlek/ceket sorun değil; kalın palto kapağa değer. Rahat "
            f"kullanım için {ASKI['on_bakan_min_govde']:.0f} mm gövde gerekir."
            if one else
            "Öne bakan askı İMKANSIZ (net iç derinlik "
            f"{ic_derinlik:.0f} mm < {ASKI['on_bakan_min_ic']:.0f} mm). "
